...
//WEB-Server
  Serial.begin(115200);
  // Initialisierung der Ausgabevariablen als Ausgaenge
...
//WEB-Server  
  ...
  if (client) {                             // Wenn ein neuer Client eine Verbindung herstellt,
    ...
    String currentLine = "";                // Erstellen eines Strings fuer eingehende Daten vom Client
    while (client.connected()) {            // Schleife, waehrend der Client verbunden ist
      if (client.available()) {             // wenn es Daten vorhanden, die vom Client gelesen werden koennen,
        ...
        if (c == '\n') {                    // Wenn das Byte ein Zeilenumrandzeichen ist
          ...
          if (currentLine.length() == 0) {
            ...
            // und ein Inhaltstyp, damit der Client weiss, was kommt, dann eine leere Zeile:
...
//************************************************************************ HTML **************************************************************************************            
            ...
            client.println(".button { background-color: #0085ff; border: none; color: white; padding: 16px 40px;");   //background-color: #4CAF50; //gruen //#e33900 rot //#0085ff blau
            client.println("text-decoration: none; font-size: 30px; margin: 2px; cursor: pointer;}");
            client.println(".button2 {background-color: #555555;}</style></head>");     //background-color: #555555; //grau
            
            // Webseitenueberschrift
            client.println("<body><h1>KHF Web Server</h1>");

            
//************************************************************************ GPIO 32 Wohnzimmer Fernsehleuchte *************************************************************

          // Aktuelle Anzeige des aktuellen Zustands und ON/OFF-Tasten fuer GPIO 32  
...
//************************************************************************ GPIO 33 Wohnzimmer Stehleuchte **************************************************************************
          ...
          // Aktuelle Anzeige des aktuellen Zustands und ON/OFF-Tasten fuer GPIO 33  
...
//************************************************************************ GPIO 25 Schlafzimmer-Deckenleuchte *************************************************************************
            ...
            // Aktuelle Anzeige des aktuellen Zustands und ON/OFF-Tasten fuer GPIO 25  
...
//************************************************************************ GPIO 26 Schlafzimmer-Schrankleuchte ***********************************************************
            ...
            // Aktuelle Anzeige des aktuellen Zustands und ON/OFF-Tasten fuer GPIO 26 
...
//************************************************************************ GPIO 27 Flurleuchte  **********************************************************
            ...
            // Aktuelle Anzeige des aktuellen Zustands und ON/OFF-Tasten fuer GPIO 27  
...
//************************************************************************ GPIO 34 Fernsehleuchte **************************************************************************
          ...
          // Aktuelle Anzeige des aktuellen Zustands und ON/OFF-Tasten fuer GPIO 34  
...
//**************************************************************************************************************************************************************************
            ...
            break;
          } else { // Wenn eine neue Zeile, dann loesche die currentLine
            currentLine = "";
          }
        } else if (c != '\r') {  // wenn etwas anderes als RETURN gdrueckt,
          currentLine += c;      // dann fuege es am Ende der currentLine hinzu
        }
      }
    }
    // Loeschen der Headervariablen
    header = "";
